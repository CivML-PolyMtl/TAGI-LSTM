...
%                     rnnMem_m{i} = init_m.*ones(node(i)*batchSize, 1, 'single');
        ...
        function [yPd, SyPd, mem, yPd_pos, SyPd_pos] = gruTest(net, lstm, x, y, y_nomask, RollWindow, transfer_sq)
            yPd_pos  = lstm.Sq{1};
            SyPd_pos = lstm.Sq{2};
            if isnan(RollWindow)
                [yPd, SyPd, ~, mem] =task.runGRU(net, lstm, x, y);
            else
                mem0 = lstm.Mem;
                Sq0  = lstm.Sq;
                nbWindow = ceil(size(y,1)/RollWindow);
                ...
                for i = 1:nbWindow
                    ...
                    yloop = y((i-1)*RollWindow+1:min(i*RollWindow,size(y,1)),:);
                    [xloop, yloop] = tagi.prepDataBatch_RNN (xloop, yloop, 1, net.sql);
                    [yPd_prior{i}, SyPd_prior{i},~, ~] = task.runGRU(net, lstm, xloop, yloop);
                    yloop = y_nomask((i-1)*RollWindow+1:min(i*RollWindow,size(y,1)),:);
                    [yPd_, SyPd_, ~, mem] = task.runGRU(net, lstm, xloop, yloop);
                    if transfer_sq == 1
                        yPd_pos  = [yPd_pos ;yPd_];
                        SyPd_pos = [SyPd_pos;SyPd_];
                    elseif transfer_sq == 0
