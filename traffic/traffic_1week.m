...
% initSeed = 435;
% initSeed = randi([1,1000],[1,1]);
initSeed = [212 722 354 950 67];
 
%% Data
path  = char([cd ,'/traffic']);
...
batchSize = 4;
nb_tsPar = 24; 
t1 = datenum('01-Jan-2008 01:00:00','dd-mmm-yyyy HH:MM:SS');
tend = datenum('31-Mar-2009 00:00:00','dd-mmm-yyyy HH:MM:SS');
t = [t1:1/24:tend]';
 
t = t(137:472); % 07-01-2008 to 20-01-2008
y = y(137:472,:);
...
nbval    = 1*24;
nbAdd_ValTest = sql;        % number of points adding to the validation and test set
...
for ts = 1:nb_tsPar:nbTS
    ...
    memVal_ts  = cell(nbTsLoop,net.maxEpoch);
    ytestPd_ts  = cell(nbTsLoop,1);
    SytestPd_ts = cell(nbTsLoop,1);
    memTrain_ts = cell(nbTsLoop,1);  
    %
...
%             lstm.Mem = memTrain_ts{j};
            lstm.Mem = [];
            [ytrainPd, SytrainPd, theta{j}, memTrain_ts{j}] = task.runLSTM(net, lstm, xtrain_loop, ytrain_loop);
...
%             lstmT.Mem = rnn.getMem (net.layer, memTrain_ts{j}, batchSize);
%             lstmT.Sq  = rnn.getSq (sql, ytrainPd, SytrainPd-svGrid(epoch)^2);
%             lstmT.Mem = rnn.getMem (net.layer, memTrain_ts{j}, batchSize);
%             lstmT.Sq  = rnn.getSq (sql, ytrain_loop, zeros(size(ytrain_loop)));
            
            [yvalPd{j}, SyvalPd{j}, memVal{j}] = rnn.lstmTest(netT, lstmT, xval, yval_ts(:,j), yvalnomask_ts(:,j), RollWindow, transfer_sq);
%             figure(1)
...
%             clf('reset')
            % LLval
            [yvalPd_, SyvalPd_] = dp.denormalize(yvalPd{j}, SyvalPd{j}, mytrain_ts(j), sytrain_ts(j));
    ...
    memVal_ts  = memVal_ts(idx_optim); % check
    ...
    parfor j = 1:nbTsLoop
        ...
        lstmT.Mem = [];
        lstmT.Sq  = rnn.getSq (sql, yvalnomask_ts(:,j), zeros(size(yvalnomask_ts(:,j))));
%         lstmT.Mem = memVal_ts{j};
%         lstmT.Sq  = rnn.getSq (sql, yvalPd_ts{j}, SyvalPd_ts{j}-sv_low^2);
%         lstmT.Mem = memVal_ts{j};
%         lstmT.Sq  = rnn.getSq (sql, yvalnomask_ts(:,j), zeros(size(yvalnomask_ts(:,j))));
...
save('traffic_1week_ensemSvFix05_verify','y','t','ytestPd','SytestPd','initSeed','epoch_optim','testIdx','tsIdx','net','netT','ytestPd_cell','SytestPd_cell')
